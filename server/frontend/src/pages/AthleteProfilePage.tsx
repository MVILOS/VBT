...
export default function AthleteProfilePage() {
  ...
  if (!athlete) return (
    <div className="text-red-400 p-8">Athlete not found.</div>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/athletes')} className="text-gray-400 hover:text-white text-sm">
          ← Athletes
        </button>
        ...
        <span className={`px-3 py-1 rounded text-xs font-medium ${athlete.is_active ? 'bg-green-900/40 text-green-300' : 'bg-gray-700 text-gray-400'}`}>
      ...
      {tab === 'plans' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-semibold text-white">Assigned Plans</h2>
            <button
          ...
          {plans.length === 0 ? (
            <p className="text-gray-400 text-sm py-8 text-center">No plans assigned yet.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {plans.map(plan => (
                <div key={plan.id} className="bg-gray-800 border border-gray-700 rounded-lg p-4">
                  ...
                  <button
                    ...
                    className="mt-3 w-full px-3 py-1.5 bg-violet-900/40 hover:bg-violet-900/60 text-violet-300 text-sm rounded transition-colors"
                  >
      ...
      {tab === 'sessions' && (
        <div className="space-y-3">
          ...
          {sessions.length === 0 ? (
            <p className="text-gray-400 text-sm py-8 text-center">No sessions recorded yet.</p>
          ) : (
            sessions.map(s => (
              <div key={s.id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex items-center justify-between">
                <div>
                  <p className="text-white font-medium">{new Date(s.started_at).toLocaleDateString('pl-PL', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md border border-gray-700">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Training Plan (optional)</label>
                <div className="flex gap-2">
                  <select value={entryForm.plan_id}
                    onChange={e => setEntryForm({...entryForm, plan_id: e.target.value})}
                    className="flex-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm focus:outline-none focus:border-violet-600">
                  ...
                  {entryForm.plan_id && (
                    <button
                      ...
                      className="px-3 py-2 bg-violet-900/50 hover:bg-violet-900/80 text-violet-300 text-xs rounded whitespace-nowrap transition-colors"
                    >
                      ✏️ Edit Plan
                    </button>
                  )}
              ...
              <div>
                ...
                <textarea value={entryForm.notes}
                  ...
                  rows={2}
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm focus:outline-none focus:border-violet-600 resize-none" />
              </div>
                  ...
                  {['scheduled','completed','skipped'].map(s => (
                    <button key={s} onClick={() => markStatus(editingEntry.id, s)}
                      className={`flex-1 py-1 rounded text-xs capitalize transition-colors ${
                        editingEntry.status === s ? 'bg-violet-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}>
                      {s}
                    </button>
                  ))}
            ...
            <div className="flex gap-2 mt-5">
              {editingEntry && (
                <button onClick={() => deleteEntry(editingEntry.id)}
                  className="px-3 py-2 bg-red-900/40 hover:bg-red-900/60 text-red-300 text-sm rounded transition-colors">
                  Delete
                </button>
              )}
              <button onClick={() => setShowEntryModal(false)}
                className="flex-1 py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm rounded transition-colors">
                Cancel
              </button>
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg p-6 w-full max-w-sm border border-gray-700">
            <h3 className="text-lg font-bold text-white mb-4">Assign Plan to {athlete.username}</h3>
            <select value={selectedPlanId} onChange={e => setSelectedPlanId(e.target.value)}
              className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white mb-4 focus:outline-none focus:border-violet-600">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowAssignModal(false)}
                className="flex-1 py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm rounded">Cancel</button>
